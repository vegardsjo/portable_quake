...
void PF_error (void)
{
	...
	s = PF_VarString(0);
	Con_Printf ("======SERVER ERROR in %s:\n%s\n", pr_get_string(pr_xfunction->s_name), s);
...
// check to see if model was properly precached
	...
	e->v.model = G_STRING_REF(OFS_PARM1);
...
void PF_vectoyaw (void)
{
	...
	{
		yaw = (int) (atan2f(value1[1], value1[0]) * 180.f / M_PI);
		if (yaw < 0)
			yaw += 360;
	}
...
void PF_ftos (void)
{
	...
	G_INT(OFS_RETURN) = pr_add_ex_string(pr_string_temp);
}
...
void PF_makestatic (void)
{
	...
	MSG_WriteByte (&sv.signon, SV_ModelIndex(pr_get_string(ent->v.model)));
